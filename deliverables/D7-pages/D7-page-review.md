...
Principle (from user review, round 3): the original two-card table stays exactly as it is. Only two lines change inside the cards. Everything else sits below the fold.

| # | Problem → change (exact copy) | Why it books more demos | A/B test (guardrail) |
|---|---|---|---|
| 1 | "$1,250 of credits" means nothing to a buyer. → One line under each price: **"≈ 50–250 finished videos a month"** / **"≈ 100–500+ finished videos a month"** [verify]. | It answers "how much video do I get?" before the call, so the buyer arrives ready to book, not ready to email. | Line on vs off. Guardrail: share of sales calls where scope is disputed. Range = $1,250 ÷ ~$5–$25 per video [verify the figure and that Enterprise credits equal the price]. |
| 2 | "Your first month at $600" doesn't say what the risk is. → **"Your first month at $600 · no lock-in"** | The lowest-risk first step is stated where the price is read. | Copy swap. Guardrail: pilot → paid conversion at day 45. |
| 3 | No proof on the page. → **"Teams on Poolday"** logo row under the plans. | Proof sits where the decision is made. | On vs off. Guardrail: clicks on the plan-card CTAs. |
| 4 | $1,250 gets compared with a $30 SaaS seat. → Below the fold: **"Poolday vs the usual options. Priced per finished video."** Three rows (how you pay, commitment, your brand) against agency / freelancer / in-house editor. | It moves the anchor to production spend, where $1,250 a month is small. | Table vs none. Guardrail: clicks on the plan-card CTAs. The competitor cells stay qualitative. |
| 5, 6 | Nothing for procurement or for questions before the call. → One line: **"Ready for procurement on Enterprise: SSO · MSA · private Slack · API access · unlimited users. Security documentation on request"** [verify], then a four-question FAQ (credit, the $600 month, running out, needing a designer). | It unblocks larger accounts and answers the questions that otherwise become an email instead of a booking. | Test as one "below-fold pack". Guardrail: booking completion. |

Dropped after review: the pilot band, the lede under the H1 and the persona captions. The first screen now reads exactly like the original, plus two short lines.
...
| 2 | The 18-video feed is a mosaic: mixed sizes, two chips on every tile, no order. → **Four use-case rows** (Launch films · Feature videos · Podcast clips · Ads), 4 uniform 16:10 tiles each, clean video with no overlays, and **one caption under each tile**: customer name + recipe ("PostHog  Made with brand kit + single prompt"). The filter tabs stay. | A visitor finds "their" video in one glance ("we need launch films"), and seeing their use case is what makes them want the call. The page gets calmer and more premium, and the video stays the only colour. | Grouped grid vs mosaic. Guardrail: clicks into videos / plays with sound; LCP (16 tiles instead of 18). |
| 3 | No CTA between the videos and the page's end, which is where interest peaks. → After the grid: **"Your brand, in videos like these. It learns your brand once."** + **Book a 15 min demo**. | It asks at the moment of peak interest. | On vs off. Guardrail: scroll depth to Meet Poolday. |
...
**Other ways to organise the feed** (not built; test after the grouped grid):
- **Category rows that scroll sideways.** One row per use case, 3.5 tiles visible and the rest swiped. More videos fit in less height, and each row reads like a channel.
- **Grid ordered by the visitor's segment.** Reorder the rows by referrer, UTM or the last tab clicked (for example, podcast traffic sees Podcast clips first). No new design, only ordering, and it should lift relevance for paid traffic.
- **One hero reel.** Replace the grid above the fold with a single 30-second reel cut from the best 8 customer videos, with the grid below it. This is the simplest possible first screen, but it loses the "real output, many brands" proof density, so test it against the grid.
